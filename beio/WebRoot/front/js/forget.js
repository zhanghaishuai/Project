...
$(function(){
	...
	$('#login_findPwd_btn1').click(function(){
		var flag = true;
		if (new RegExp(regex('mobile')).test($('#find_mobile').val()) == false) {
			$('#find_mobile_s').removeClass('hide');
			$('#find_mobile_s').html('<i class="i-def"></i>' + tip('121'));
			flag = false;
		}
		if (new RegExp(regex('empty')).test($('#find_mobile').val()) == false) {
			$('#find_mobile_s').removeClass('hide');
			$('#find_mobile_s').html('<i class="i-def"></i>' + tip('120'));
			flag = false;
		}
		if (new RegExp(regex('empty')).test($('#find_code').val()) == false) {
			$('#find_code_s').removeClass('hide');
			$('#find_code_s').html('<i class="i-def"></i>' + tip('124'));
			flag = false;
		}
		if (flag == true) {
			$.ajax({
				url : '/beio/sys/findPwdFillMobile',
				data : {
					'mr.mobile' : $('#find_mobile').val(), 
					'mr.imgVerifyCode' : $('#find_code').val()
				},
				type : 'POST',
				async : false,
				cache : true,
				...
				success : function(data) {
					...
					if (data.status == '200') {
						...
						$('#valid_mobile').html($('#find_mobile').val());
