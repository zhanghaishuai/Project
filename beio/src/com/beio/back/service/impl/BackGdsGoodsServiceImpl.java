...
public class BackGdsGoodsServiceImpl extends BaseIbatisServiceImpl implements
	...
	public boolean addGoods(BackGdsGoodsFileVO gfv) throws Exception {
		...
		String imgsPath = ConfigUtil.getProperties("IMGSPATH")
				+ "/"
				+ DateUtil.formatDate(DateUtil.getDate(),
		...
		for (int i = 0; i < gfv.getShow().length; i++) {
			if (null != gfv.getShow()[i]
					&& null != gfv.getShowFileName()[i]
					&& -1 != gfv.getShowFileName()[i].indexOf(".")
					&& 1 < gfv.getShowFileName()[i].length()
							- gfv.getShowFileName()[i].lastIndexOf(".")) {
		...
		for (int i = 0; i < gfv.getDetaile().size(); i++) {
			if (null != gfv.getDetaile().get(i)
					&& null != gfv.getDetaileFileName().get(i)
					&& -1 != gfv.getDetaileFileName().get(i).indexOf(".")
					&& 1 < gfv.getDetaileFileName().get(i).length()
							- gfv.getDetaileFileName().get(i).lastIndexOf(".")) {
	...
	public boolean updateImage(BackGdsImageVO biv) throws Exception {
		...
		// temp.setOrgPath(orgPath + "/" +
		// getNewFileName(biv.getImgFileName()));
		// temp.setBigPath(bigPath + "/" +
		// getNewFileName(biv.getImgFileName()));
		// temp.setMidPath(midPath + "/" +
		// getNewFileName(biv.getImgFileName()));
		// temp.setSmaPath(smaPath + "/" +
		// getNewFileName(biv.getImgFileName()));
	...
	public List<BackGdsClassifyVO> allClassify(BackGdsClassifyVO bcv)
			throws Exception {
		List<BackGdsClassifyVO> dbList = selectList("backGoods.allClassify",
				bcv);
